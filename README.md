...
The `-t postgres` command will dump the table schemes and records as valid `CREATE TABLE` and `INSERT INTO` statements to stdout. This dump can then be imported in a PostgreSQL database. Each record becomes its own `INSERT` statement. Values are written as standard SQL string literals: single quotes are doubled and backslashes are kept as they are. This is correct when the [`standard_conforming_strings`](https://www.postgresql.org/docs/current/runtime-config-compatible.html#GUC-STANDARD-CONFORMING-STRINGS) option is on, which is the default since PostgreSQL 9.1. Do not import the dump with that option turned off.
