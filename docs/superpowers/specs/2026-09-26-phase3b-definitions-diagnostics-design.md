...
**Date:** 2026-09-26
**Status:** design approved by Ben (2026-09-26), section by section; this written spec is reviewed by Fable, then
executed unattended, as Ben asked
...
spec; `_diagnostic.py` and the readers' `report` callback (B1); `inspect` through `Report`, with its stderr golden files
regenerated (B2); the CP-1251 policy (B3); the annotations (B4); the API golden files and realdata fingerprints; the
removal of the dead methods and the moved tests (B5); the documentation; and the outcome.
